...
$(function() {
	...
	//Анимация складывания карт
	var cards = new Waypoint({
		element: document.querySelector('.select'),
		handler: function (direction) {
			if (direction == 'down') {
				var delta;

				delta = $('.select__container').outerHeight() - secondHeight;
				...
				$('.compare__wrap').addClass('compare__wrap--show');
				//this.disable();
			}	
		}
	})

	//Анимация расскладывания карт
	var button = new Waypoint({
		element: document.querySelector('.compare'),
		handler: function (direction) {
			if (direction == 'up') {
				
				$('.compare__wrap').removeClass('compare__wrap--show');
	...
	function compareItems() {
		...
		campareItemsFlag = false;

		compareList.each(function (i, element) {
			$(element).addClass('visible-hidden');
			new Waypoint({
				element: element,
				handler: handler,
				offset: 'bottom-in-view',
	...
	function handler(direction) {
		if (direction == 'down') {
			...
			if (this === this.group.last()) {
				//Уменьшение кнопки
				$('.compare__button').addClass('compare__button--small');
				$('.compare__wrap').addClass('compare__wrap--rel');
			...
			if (this === this.group.last()) {
				$('.compare__button').removeClass('compare__button--small');
				$('.compare__wrap').removeClass('compare__wrap--rel');
	...
	parallax('.cards', [{ obj: '.cards__gold', mov: '-20' },{ obj:'.cards__standard', mov: '-30' }]);

	//Parallax эффект на последнем слайде
	parallax('.buy', [{ obj: '.phone__card', mov: '-20' }, { obj: '.phone__phone', mov: '-40' }]);
	...
	function colorCounter(oldC, newC, fn) {
		...
		if (oldC > newC) {
			setTimeout(colorCounter.bind(this, oldC - 1, newC, fn), 50);
		} else {
			setTimeout(colorCounter.bind(this, oldC + 1, newC, fn), 50);
		}
	}
	...
	secondLeave_.eventCallback("onComplete", function() {
		Waypoint.refreshAll();

		// button.enable();
		// cards.enable();
	})
	secondLeave640_.eventCallback("onComplete", function () {
		Waypoint.refreshAll();
		// button.enable();
		// cards.enable()
	})	
	secondLeave_.eventCallback("onReverseComplete", function () {
		Waypoint.refreshAll();
	})
	secondLeave640_.eventCallback("onReverseComplete", function () {
		Waypoint.refreshAll();
	})		
