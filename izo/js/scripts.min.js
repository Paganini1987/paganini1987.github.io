...
$(function() {
	...
	var fp = $('#fullpage').fullpage({
		...
		fixedElements: '.header__top, .compare__wrap--fix',
		...
		onLeave: function (index, nextIndex, direction) {
			...
			if (nextIndex == 3 && direction == 'down') {
				...
				$('.compare__button--fix').addClass('show-button');
				$('.compare__container').animate( {scrollTop: '15px'}, 200);
				//Скрыть меню
	...
	function handler(direction) {
		if (direction == 'down') {
			...
			if (this === this.group.last()) {
				var compareHeight = $('.compare__container').outerHeight(),
					selectHeight = $('.select__container').outerHeight(),
					windowHeight = $(window).height();

				totalHeight = compareHeight + selectHeight;

				totalHeight = Math.round(totalHeight);

				console.log(totalHeight + windowHeight - compareHeight);
					
				//Скрыть меню
				$('.fullpage-wrapper').css('transform', 'translate3d(0px, ' + (-Math.ceil(totalHeight + windowHeight - compareHeight)) + 'px, 0px)');
				//Уменьшение кнопки
				$('.compare__button--fix').removeClass('show-button');
			...
			if (this === this.group.last()) {
				var selectHeight = $('.select__center').outerHeight();

				$('.compare__button--fix').addClass('show-button');

				//Показать меню
				$('.fullpage-wrapper').css('transform', 'translate3d(0px, ' + (- totalHeight) + 'px, 0px)');
	...
	var button = new Waypoint({
		...
		enabled: false,
		offset: function() {
			var margin = $('.js-show-button').css('margin-top');
	...
	parallax('.phone', [{ obj: '.phone__card', mov: '-20' }, { obj: '.phone__phone', mov: '-40' }]);
	...
	secondLeave_
		...
		.to('.select__cards--large', 1, {
			transform: 'translateY(-180px)',
			opacity: 0
		}, 'start')
		.to('.select__cards--large', 0, {
			display: 'none'
		})
		.to('.select__cards--min', 0, {
			display: 'flex',
			transform: 'translateY(180px)',
			opacity: 0
		})
		.to('.select__cards--min', 1, {
			transform: 'translateY(0px)',
			opacity: 1
		})
