...
$(function() {
	var load = false,
		promises = [],
		images = ['img/wallet.png', 'img/card-gold.png', 'img/pocket.png','../img/main_background.jpg'];
	//Прелоадер
	$(window).on('load', init);

	promises = images.map(function(item) {
		return p(item);
	})
	...
	function init() {
		...
		$('.loader').hide();
		showElement('.wallet__wrap', 'fadeIn');
		$('.wallet__line').css('animation-play-state', 'running');
		load = true;
	}
	...
	//Card
	var card = new Waypoint({
		element: document.querySelector('.wallet__card'),
		handler: function (direction) {
			$('.wallet__card').addClass('out');

			this.disable();
		},
		offset: function() {
			return $('.wallet__card').offset().top - 5;
		}
	})
	...
	$(window).on('scroll', function() {
		...
		var steps = new Waypoint({
			...
			handler: function (direction) {
				showElement('.steps__item', 'fadeInUp');
	...
	var advantages = new Waypoint({
		element: document.querySelector('.advantages__list'),
		handler: function (direction) {
			showElement('.advantages__item', 'fadeInUp');
	...
	//Collect
	var collect = new Waypoint({
		element: document.querySelector('.collect__title'),
		handler: function (direction) {
			showElement('.collect__title', 'fadeInUp');
			showElement('.collect__text', 'fadeInUp');
	...
	var collect2 = new Waypoint({
		element: document.querySelector('.collect__list'),
		handler: function (direction) {
			showElement('.collect__note', 'fadeInUp');
			showElement('.collect__item', 'fadeInUp');
	...
	//Spend
	var spend = new Waypoint({
		element: document.querySelector('.spend__line1'),
		handler: function (direction) {
			showElement('.spend__title', 'fadeInUp');
			showElement('.spend__note', 'fadeInUp');
	...
	function showElement(obj, animation) {
		$(obj).removeClass('visibility-hidden');
		$(obj).addClass('animated');
		$(obj).addClass(animation);
	}

	//Логика работы калькулятора и отображения карточек
	...
	var calculator = new Vue ({
		...
		computed: {
			result: function() {
				...
				if ((this.total - this.cafe) * this.percentOther > this.maxOther) {
					bTotal = this.maxOther;
				} else {
					bTotal = (this.total - this.cafe) * this.percentOther;
				}

				if (this.cafe * this.percentPromo > this.maxPromo) {
					bCafe = this.maxPromo;
				} else {
					bCafe = this.cafe * this.percentPromo;
				}

				this.summ = (bTotal + bCafe) * this.month;

				return String(this.summ).replace(/(\d)(?=(\d{3})+([^\d]|$))/g, '$1 ');
			}
		},
		mounted: function() {
			//Селекторы суммы
			...
			rangeSlider.noUiSlider.on('update', function (value) {
				...
				//Если траты в кафе больше суммарных трат - делаем их равными
				if (value < that.cafe) {
					that.cafe = value;

					rangeSliderCafe.noUiSlider.set(value);
				}
				result = String(value).replace(/(\d)(?=(\d{3})+([^\d]|$))/g, '$1 ')
				$('.js-set-total-value').html(result);

				that.total = value;
			});

			//Cafe
			var rangeSliderCafe = document.getElementById('range2');

			noUiSlider.create(rangeSliderCafe, {
				start: [10000],
				step: 5000,
				connect: [true, false],
				range: {
			...
			rangeSliderCafe.noUiSlider.on('update', function (value) {
				var value = Math.round(value),
					result;
	...
	var list = new Vue({
		...
		data: {
			...
			cards: [],
			filterCat: {},
			category: 'Дом и дети' //Категория по умолчанию
		},
		methods: {
			getData: function () {
				var that = this;
				...
				axios.get('cards.json')
					.then(function (response) {
						response.data.map(function(item) {
							if (that.filterCat[item.category] == undefined) {
								that.filterCat[item.category] = [item.partner];
							} else {
								if (!that.filterCat[item.category].some(function (partner) { return partner == item.partner })) {
									that.filterCat[item.category].push(item.partner);
								}
							}
						})

						//Рандомно выбираем 4 партнёров в каждой категории
						for (var cat in that.filterCat) {
							var length = that.filterCat[cat].length,
								randomIndex = [],
								index,
								j = 0;
							...
							do {
								...
								if (!randomIndex.some(function(index_) { return index_ == index })) {
									randomIndex.push(index);
								}
							} while (randomIndex.length < 4);

							that.filterCat[cat] = that.filterCat[cat].filter(function(partner) { 
						...
						that.cards = response.data.filter(function(item) {
							if (that.filterCat[item.category].some(function(partner) {
			...
			loadMore: function() {
				...
				if (this.items >= this.cardsList.length) {
					this.cardsOver = true;
				}
			},
			beforeEnter: function(el) {
				//$(el).addClass('beforeEnter');
			},
			enter: function(el) {
				$(el).addClass('flipInY');
			},
			...
			leave: function(el) {
				$(el).addClass('flipOutY');
			},
			afterLeave: function (el) {
				$(el).removeClass('flipOutY');
			}
		},
		computed: {
			cardsList: function () {
				...
				cards = this.cards.filter(function (item) {
					return item.category == that.category && item.cost <= calculator.$data.summ;
				}).sort(function (a, b) {
				...
				if (cards.length <= this.items) {
					this.cardsOver = true;
				} else {
					this.cardsOver = false;
				}
		...
		mounted: function() {
			...
			$('body').on('click', '.js-set-active', function () {
				$('.js-set-active').removeClass('active');
				$(this).addClass('active');
			})

			//Вейпоинт для постепенного появления карточек
			this.$nextTick(function() {
				...
				var cards = new Waypoint({
					element: document.querySelector('.cards__categories'),
					handler: function (direction) {
						$('.cards__card').each(function (i, item) {
							var id = +$(item).data('id');

							$(item).addClass('flipInY');
							$(item).css('animation-delay', (id * 100) + 'ms');
						})
	...
	// setTimeout(function() {
	// 	$('.cards__card').each(function (i, item) {
	// 		$(item).removeClass('flipInY');
	// 	})
	// }, 1000)

	//Выбор категории
	var categories = $('#categories').selectize();
	var selectize = categories[0].selectize;

	categories.on('change', function (e) {
		router.setRoute($(e.target).val());
	})

	//Роутер
	var home = showCategory.bind(null, 'Дом и дети'),
		beauty = showCategory.bind(null, 'Красота и здоровье'),
		clothes = showCategory.bind(null, 'Одежда и аксессуары'),
		electronics = showCategory.bind(null, 'Электроника'),
		sport = showCategory.bind(null, 'Спорт и развлечения');
	...
	function showCategory(category) {
		...
		list.$data.cardsOver = false;
		list.$data.items = list.$data.minItems;

		//Подсветка активной категории
		$('.js-set-active').removeClass('active');

		$('.js-set-active').each(function(i, item) {
			var catName = $(item).find('span').text(),
				catVal = $(item).attr('href').substr(2);
				
			if (catName == category) {
				$(item).addClass('active');

				//Выбор активной категории в списке (на малых разрешениях)
				selectize.setValue(catVal, true);
			}
		})
	}
	...
	var counter = new Vue({
		...
		methods: {
			...
			setNowPoints: function() {
				var now = new Date();
				
				this.nowPoints = this.startPoints + ((now - this.startDate) / 1000) * this.pointsPerSecond;
				this.nowClients = this.startClients + ((now - this.startDate) / (1000 * 60 * 60)) * this.clientsPerHour;
			...
			setRandomInterval: function() {
				...
				this.interval = Math.round(Math.random() * (max - min) + min);
			}
		},
		filters: {
			pointsFormat: function(data) {
		...
		mounted: function() {
			this.setNowPoints();
			this.counter(this.setNowPoints);
		}
	})
	...
	$('#month').on('change', function(e) {
		calculator.$data.month = $(e.target).val();	
	})

	//Выбор карт
	$('.js-show-standard').on('click', function(e) {
		e.preventDefault();

		$('.wallet-and-cards').removeClass('show-gold');
		$('.wallet-and-cards').addClass('show-standard');
		$('.online__button').prop('disabled', false);
		$('.online__button').text('заказать карту');
		$('.online__button').data('card','standard');
	})

	$('.js-show-gold').on('click', function (e) {
		e.preventDefault();

		$('.wallet-and-cards').removeClass('show-standard');
		$('.wallet-and-cards').addClass('show-gold');
		$('.online__button').prop('disabled', false);
		$('.online__button').text('заказать карту');
		$('.online__button').data('card', 'gold');
	})
	...
	$('.js-open-modal').on('click', function () {
		...
		if (cardType == 'gold') {
			iframe_src = 'https://appl.unicredit.ru/credit_cards?product_type=MC_Gold' + postfix;
		}
		if (cardType == 'standard') {
			iframe_src = 'https://appl.unicredit.ru/credit_cards?product_type=MC_Standard' + postfix;
		}
	...
	$('a.scroll').on('click', function (e) {
		...
		if (this.hash == '#conditions') {
			$('html, body').animate({ scrollTop: Math.round(target_offset) - ($(window).height() - $(this.hash).height())+93 }, 1000);

			return null;
		}
		$('html, body').animate({ scrollTop: Math.round(target_offset) - customoffset }, 1000);
	});
