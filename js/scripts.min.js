...
//Анимация фона в хедере
//Рандомные значения конечной точки маршрута градиента
...
var grad1 = TweenMax.to($('#grad1'), 15, {
    attr: {
        cx: randomTarget() + '%',
        cy: randomTarget() + '%'
    },
...
var grad2 = TweenMax.to($('#grad2'), 35, {
    attr: {
        cx: randomTarget() + '%',
        cy: randomTarget() + '%'
    },
...
var grad3 = TweenMax.to($('#grad3'), 15, {
    attr: {
        cx: randomTarget() + '%',
        cy: randomTarget() + '%'
    },
...
var grad4 = TweenMax.to($('#grad4'), 25, {
    attr: {
        cx: randomTarget() + '%',
        cy: randomTarget() + '%'
    },
...
var grad5 = TweenMax.to($('#grad5'), 18, {
    attr: {
        cx: randomTarget() + '%',
        cy: randomTarget() + '%'
    },
...
function enterFirst() {
    ...
    slide1
        ...
        .set('.banner__title, .banner__text, .banner__item, .cards__gold, .cards__standard', {
    ...
    //Отключаем анимацию фона при появлении элементов на первом слайде
    slide1.eventCallback("onStart", function () {
        allGradientsPause();
    });
    //Включаем анимацию фона только после завершения анимации появления
    slide1.eventCallback("onComplete", function () {
        allGradientsPlay();
    });
}
//2 слайд Выбор карты
...
function sixthEnter(speed) {
    ...
    sixthEnter
        .set('.buy__text, .buy__title, .buy__icons', {
            transform: 'translateY(-50px)',
            opacity: 0
        })
        ...
        .to('.phone__card, .phone__shadow, .phone__phone', 1, {
            transform: 'translateY(0px)',
            opacity: 1
        }, '+=0.5')
        .to('.buy__note', speed, {
...
$(function() {
	...
	//Анимации входа и ухода со слайдов расположены в файле animations.js

	$('#fullpage').fullpage({
		sectionSelector: '.js-section',
		verticalCentered: false,
		fixedElements: '.header__top, .compare__wrap',
		normalScrollElements: '.compare__container',
		scrollingSpeed: 1500,
		easingcss3: 'ease-in-out',
		onLeave: function (index, nextIndex, direction) {
			nextSlide = nextIndex;

			//Анимация на 2 слайде при переключении на 3
			if (nextIndex == 3 && direction == 'down') {
				secondLeave();

				button.enable();
				//Скрыть меню
				TweenMax.to('.header__top', 1.5, {
					transform: 'translateY(-100px)',
					opacity: 0
				}, 'start')
			...
			if ((index == 3 || index == 4) && direction == 'up') {
				
				//Позиционирование градиента
				posGradient(selector.activeCard);
			}
			//С первого на второй
			if (index == 1 && nextIndex == 2) {
				secondEnterFromFirst(1);
			}

			//Со второго на первый
			if (index == 2 && nextIndex == 1) {
				secondLeaveToFirst(1);

				//Убрать градиент у карт

				var goldGradient = $('.gold-gradient'),
					standardGardient = $('.standard-gradient');

				var gradOutS = TweenMax.to(standardGardient, 0.5, {
					attr: {
						r: "0%"
					},
					ease: 'Linear.easeNone'
				});
				var gradOutG = TweenMax.to(goldGradient, 0.5, {
					attr: {
						r: "0%"
					},
					ease: 'Linear.easeNone'
			...
			//Анимация на 2 слайде при переключении c 3
			if (nextIndex == 2 && direction == 'up') {
				if (!allowTop) {
					return false;
				}
				secondEnter();
				$('.compare__button').removeClass('show-button');
				//Показать меню
				TweenMax.to('.header__top', 1.5, {
					transform: 'translateY(0px)',
					opacity: 1
			...
			//Отключить анимацию градиента при смене слайда, чтобы не тормозило
			allGradientsPause();
		},
		afterLoad: function (anchorLink, index) {
			var loadedSection = $(this);

			//Включить анимацию градиента на 1 и 2 слайдах
			if (index < 3) {
				allGradientsPlay()
			}

			if (index == 4) {
				$('.compare__container').mCustomScrollbar('scrollTo', '1%');
			}
	...
	var get = new Waypoint({
		...
		context: document.querySelector('.compare__container')
	})
	...
	var compareList = $('.compare__item');

	compareList.each(function (i, element) {
		new Waypoint({
			element: element,
			handler: handler,
			offset: 'bottom-in-view',
	...
	function handler(direction) {
		if (direction == 'down') {
			...
			$(this.element).removeClass('visible-hidden');

			//Скрыть кнопку и меню
			...
			//Паказать кнопку и меню
			if (this === this.group.last()) {
				$('.compare__button').addClass('show-button');
	...
	//Появление кнопки, убрать кнопку и меню
	var button = new Waypoint({
		element: document.querySelector('.js-show-button'),
		handler: function (direction) {
			if (direction == 'down') {
				$('.compare__button').addClass('show-button');
				console.log('show');
	...
	//Уменьшение кнопки
	var buttonSmall = new Waypoint({
		element: document.querySelector('.js-show-small'),
		handler: function (direction) {
			if (direction == 'down') {
				$('.compare__button').addClass('button-small');
				console.log('small');
				
			} else {
				$('.compare__button').removeClass('button-small');
			}
		},
		offset: 'bottom-in-view',
		context: document.querySelector('.compare__container')
	...
	$('.select__card input').on('change', function() {
		if ($(this).prop('checked') == true) {
			...
			//Позиционироавние градиента под картой
			posGradient(type);
		}
	})

	function posGradient(card) {
		var obj,
			width,
			height,
			x,
		...
		x = obj.offset().left + width / 2;
		y = obj.offset().top + $(window).height() + height / 2;


		if (card == 'gold') {
			var gradIn = new TimelineMax();
			...
			var gradOut = TweenMax.to(standardGardient, 1, {
				attr: {
					r: "0%"
				},
				ease: 'Linear.easeNone'
			});
			var grad = TweenMax.to($('.end-point'), 1, {
				attr: {
					'stop-color': "#996699"
				},
				ease: 'Linear.easeNone'
		...
		if (card == 'standard') {
			...
			var gradOut = TweenMax.to(goldGradient, 1, {
				attr: {
					r: "0%"
				},
				ease: 'Linear.easeNone'
	...
	// $(".compare__container").mCustomScrollbar({
	// 	callbacks: {
	// 		whileScrolling: function () { 
	// 			if (this.mcs.topPct >= 70) {
	// 				$('.js-checkout').addClass('button-small');
	// 			}

	// 			if (this.mcs.topPct <= 70) {
	// 				$('.js-checkout').removeClass('button-small');
	// 			}
	...
	// 			$('.compare__item').each(function(i, item) {
	// 				var top = $(item).offset().top,
	// 					contHeight = $('.compare__container').height();

	// 				//console.log(contHeight, top, item);
					
	// 				if (top <= contHeight - 100) {
	...
	// 			if (this.mcs.topPct != 100 && nextSlide == 3) {
	// 				$('.compare__button').addClass('show-button');
	// 			}
	...
	function secondEnter() {
		var secondEnter = new TimelineMax();
		...
		secondEnter
			...
			.to('.select__description', 0.7, {
				maxHeight: '0px',
				maxWidth: '0px',
				opacity: 0
			}, 'start')
			.to('.select__card label', 0, {
				display: 'block',
			})
			.to('.select__description', 0, {
				marginLeft: '0px',
				marginTop: '6vh',
				marginBottom: '4vh'
			})
	...
	function secondLeave() {
		var secondLeave = new TimelineMax();
