<template>
	<a href="javascript:void(0);" class="dish">
		<ul class="dish__labels">
			<li class="dish__label" v-for="(label, index) in labels" :key="index">{{ label }}</li>
		</ul>
		<div class="dish__img">
			<img :src="inputData.img" :alt="inputData.description">
		</div>
		...
		<div class="dish__footer" :class="{ 'dish__footer--set': !isSet }">
			...
			<div class="dish__info">
				...
				<span class="dish__value">{{ inputData.time }} мин.</span>										
			</div>
			<div class="dish__info">
				<svg class="dish__icon" width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
				...
				<span class="dish__value">{{ inputData.weight }} гр.</span>										
			</div>

			<template v-if="isSet">
				<div class="dish__switch df-switch df-switch--small" @click.stop="toggleSelect" :class="{ selected: inputData._selected }">
			...
			<div v-else class="dish__footer-bottom">
				...
				<div class="dish__add df-add" :class="{ isOpened: counter > 0 }">
					<div class="df-add__label">Персон</div>
					<div class="df-add__holder">
						<div class="df-add__minus" @click="remove">
						...
						<div class="df-add__counter">{{ counter }}</div>
					</div>
					<div class="df-add__wrapper" :class="{ max: isMaxValue}">
						...
						<div class="df-add__plus" @click="add" :class="{ 'df-add__plus--disabled': isMaxValue}">
...
export default {
	...
	computed: {
		labels () {
			return this.inputData.labels ? this.inputData.labels : []
		},
		isMaxValue () {
	...
	methods: {
		...
		toggleSelect () {
			this.$store.dispatch('TOGGLE_SELECTED_STATE_DISH', this.inputData.id)
		}
	},
	watch: {
